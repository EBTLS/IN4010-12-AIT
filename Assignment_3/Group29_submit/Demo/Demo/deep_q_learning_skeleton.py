...
class ReplayMemory(object):
    ...
    def __init__(self, size):
        self.size = size
        self.prev_obs = np.zeros((size,9), dtype = np.int)
        self.obs = np.zeros((size,9), dtype = np.int)
        self.actions = np.zeros(size, dtype = np.int)
        self.rewards = np.zeros(size, dtype = np.float)
        self.dones = np.zeros(size, dtype = np.int)
        self.cnt = 0

    # Store experience in memory
    def store_experience(self, prev_obs, action, observation, reward, done):
        self.prev_obs[self.cnt] = prev_obs
        self.obs[self.cnt] = observation
    ...
    def sample_batch(self, batch_size):
        rand = np.random.choice(self.size, size=batch_size, replace=False)
        prev_obs = self.prev_obs[rand]
        action = self.actions[rand]
        observation = self.obs[rand]
        reward = self.rewards[rand]
        done = self.dones[rand]
        return prev_obs, action, reward, observation, done
    
    def latest_batch(self, batch_size):
        lower_bd, upper_bd = self.cnt, self.cnt + batch_size
        prev_obs = self.prev_obs[lower_bd:upper_bd]
        action = self.actions[lower_bd:upper_bd]
        observation = self.obs[lower_bd:upper_bd]
        reward = self.rewards[lower_bd:upper_bd]
        done = self.dones[lower_bd:upper_bd]
        return prev_obs, action, reward, observation, done
...
class QNet(nn.Module):
    ...
    def single_Q_update(self, prev_observation, action, observation, reward, done, target_qn):
        ...
        if done:
            ...
            future_val = self.max_Q_value(t_observation) if target_qn is None else target_qn.max_Q_value(t_observation) ##<<- this evaluates the QNet
    ...
    def batch_Q_update(self, obs, actions, next_obs, rewards, dones, target_qn):
        batch_size = len(dones)
        v_next_obs = self.max_Q_value(next_obs, batch_size) if target_qn is None else target_qn.max_Q_value(next_obs, batch_size)
        not_dones = 1 - dones
...
class QLearner(object):
    def __init__(self, env, q_function, discount=DEFAULT_DISCOUNT, rm_size=RMSIZE, target_network=None, isReplayMem=False):
        self.env = env
        self.Q = q_function
        ...
        self.isReplayMem = isReplayMem          # employ replayMemory    
    ...
    def process_experience(self, action, observation, reward, done):
        ...
        # self.Q.single_Q_update(self.target_network, prev_obs, action, observation, reward, done)
        ...
        if self.isReplayMem:
            if self.tot_stages > 10 * self.batch_size:
                prev_obs, action, observation, reward, done = self.rm.sample_batch(self.batch_size)
                self.Q.batch_Q_update(prev_obs, action, reward, observation, done, self.target_network)
        else:
            if self.tot_stages % self.batch_size == 0:
                ## batch update
                prev_obs, action, observation, reward, done = self.rm.latest_batch(self.batch_size)
                self.Q.batch_Q_update(prev_obs, action, reward, observation, done, self.target_network)
                ## single update
                # self.Q.single_Q_update(prev_obs, action, observation, reward, done, self.target_network)
